...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        //create map with calories per day
        Map<LocalDate, Integer> datesWithCalories = mealList
                .stream()
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededCycle(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : mealList) {
            datesWithCalories.merge(meal.getDateTime().toLocalDate(), meal.getCalories(), (oldValue, newValue) -> oldValue + newValue);
        }
        ...
        for (UserMeal meal : mealList) {
            if (TimeUtil.isBetween(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                listMealWithExceed.add(new UserMealWithExceed(meal.getDateTime(), meal.getDescription(), meal.getCalories(),
